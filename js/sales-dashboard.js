...
function parseSalesCSV(text, sourceMode) {
    ...
    for (let i=headerRowIdx+1;i<records.length;i++) {
        ...
        let storeCode=normalizeStoreKey_(getStr('storeCode',row));
        // Poin 1 (pertahanan lini kedua): kalau nilai yang terbaca sebagai
        // Store Code TERNYATA identik dengan Nama BM/ABM di baris yang sama,
        // itu tanda kolom tertukar (header tidak terbaca dgn benar) -- jangan
        // dipakai. Dikosongkan (bukan diisi '-') supaya fallback ke Nama Toko
        // di getSlicerReferenceData_/applySalesFilters/sortir tetap benar.
        if (storeCode && (storeCode===normalizeStoreKey_(bmVal) || storeCode===normalizeStoreKey_(abmVal))) storeCode='';
        ...
        // Poin 3: kalau Sales LY = 0 (toko baru / tidak ada histori tahun
        // lalu), Ach SSSG & Proj SSSG dipaksa 0 -- persentase pertumbuhan
        // tidak bermakna tanpa basis pembanding.
        ...
        result.push({
            ...
            mtdSales:sales, mtdTarget:target,
            // Poin 5: Best Estimate (proyeksi total sales akhir bulan) perlu
            // berupa angka -- dipakai untuk hitung %PROJ SSSG level ringkasan.
            bestEstimate:getNum('bestEstimate',row),
...
function applyOfficialComparisonFallbacks_() {
    ...
    salesData.forEach(item=>{
        ...
        // Poin 3: item.sssg (Ach SSSG per toko) SEBELUMNYA TIDAK PERNAH
        // DIHITUNG di sini -- field-nya tidak pernah di-set sehingga selalu
        // terbaca 0 di tabel. Ach SSSG = (Sales MTD - Sales LY) / Sales LY.
        // Kalau Sales LY = 0 (toko baru), Ach SSSG & Proj SSSG dipaksa 0.
...
function getOfficialComparison_(data) {
    ...
    // Poin 4: %ACH SSSG = pertumbuhan (total MTD Sales vs total Sales LY),
    // TAPI hanya untuk toko yang PUNYA histori Sales LY. Toko baru (Sales LY
    // = 0, tidak ada basis pembanding) dikecualikan dari KEDUA sisi -- baik
    // MTD Sales-nya maupun (otomatis) Sales LY-nya -- supaya tidak membiaskan
    ...
    data.forEach(item=>{
        ...
        const sub=findSubmissionForOfficial_(item);
        const ly = sub ? (Number(sub.salesLY)||0) : 0;
        totalLY += ly;
        if (ly>0) {
            lyWithHistory += ly;
