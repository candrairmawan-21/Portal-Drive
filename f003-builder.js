...
// SMART BARCODE & SCANNER LISTENER (Mendukung Ketikan Manual & Hardware Scanner PDT)
function setupBarcodeScannerListener(rowNum) {
    ...
    barcodeInput.addEventListener("input", function (e) {
        ...
        if (lastKeyTime > 0) {
            ...
            // Jika interval antar karakter sangat cepat (< 40ms), ini dipastikan dari Hardware Scanner PDT
            if (interval < 40) {
                isScannerInput = true;
            } else {
                isScannerInput = false; // Ketikan manual oleh manusia
            }
        }
        ...
        // Hanya auto-advance jika input masuk melalui hardware scanner DAN panjang karakter sudah valid
        if (isScannerInput && barcodeInput.value.trim().length >= 4) {
            ...
            }, 250); // Jeda aman untuk memastikan seluruh string laser scanner selesai masuk tanpa terpotong
        }
    });
}
